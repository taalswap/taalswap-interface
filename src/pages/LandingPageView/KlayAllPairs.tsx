...
const KlayAllPairs = () => {
  ...
  const getTokenIconPath = (symbol) => {
    ...
    if (
      ...
      path = `${
        process.env.REACT_APP_INTERFACE
      }/images/coins/${symbol.toLowerCase()}.png`;
    } else {
      path = `${
        process.env.REACT_APP_INTERFACE
      }/images/coins/${symbol.toLowerCase()}.svg`;
    }
  ...
  const pairTableRow = () => {
    ...
    pairs.forEach((pair) => {
      if (pair.base_symbol !== 'TAL' && pair.quote_symbol !== 'TAL') {
        const base_symbol =
          pair.base_symbol === 'WKLAY' ? 'KLAY' : pair.base_symbol;
        const quote_symbol =
          pair.quote_symbol === 'WKLAY' ? 'KLAY' : pair.quote_symbol;
        ...
        const baseDeposit =
          pair.base_symbol === 'WKLAY' ? 'KLAY' : pair.base_address;
        const quoteDeposit =
          pair.quote_symbol === 'WKLAY' ? 'KLAY' : pair.quote_address;
    ...
    async function fetchData() {
      ...
      await fetch(`${getKlaytnApiUrl()}/pairs`, {
        ...
        .then((response) => {
          ...
          const array = pairArray.map((pair) =>
            JSON.parse(JSON.stringify(pair[1]))
          );
          setPairs(array);
  ...
  return (
    <div
      className="farms_wrap user_section"
      style={{ maxWidth: '1280px', padding: '50px 20px 0px 10px' }}
    >
      <div
        style={{
          display: 'flex',
