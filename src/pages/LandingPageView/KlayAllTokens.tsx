...
const KlayAllTokens = () => {
    ...
    tokens.forEach((token) => {
      if (token.symbol !== 'TAL') {
        ...
        if (
          ...
          }/images/coins/${token.symbol.toLowerCase()}.png`;
          ...
          path = `${
            process.env.REACT_APP_INTERFACE
          }/images/coins/${token.symbol.toLowerCase()}.svg`;
        }
    ...
    async function fetchData() {
      ...
      await fetch(`${getKlaytnApiUrl()}/tokens`, {
        ...
        .then((response) => {
          ...
          const array = toeknsArray.map((pair) =>
  ...
  return (
    <div
      ...
      style={{ maxWidth: '1280px', padding: '50px 10px 0px 20px' }}
    >
      <div
        style={{
          display: 'flex',
          ...
          {tokenTableRow()
            ...
            .map((token) => (
              <tr key={token.symbol}>
                ...
                >
                  <IconButton
                    onClick={() => linkToURL(token.prices)}
                    variant="text"
                    scale="sm"
                    ml="4px"
                  >
                ...
                >
                  <IconButton
                    onClick={() => linkToURL(token.deposit)}
                    variant="text"
                    scale="sm"
                    ml="4px"
                  >
