...
export const fetchPoolsTotalStaking = async () => {
  ...
  const bnbPool = poolsConfig.filter((p) => p.stakingToken.symbol === 'ETH' || p.stakingToken.symbol === 'KLAY')
...
export const fetchPoolsStakingLimits = async (
  ...
  const validPools = poolsConfig
    .filter((p) => p.stakingToken.symbol !== 'ETH' && p.stakingToken.symbol !== 'KLAY' && !p.isFinished)
