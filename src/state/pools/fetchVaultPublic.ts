...
export const fetchPublicVaultData = async () => {
  try {
    ...
    const totalCakeInVaultEstimate = convertSharesToCake(
    ...
    return {
      ...
      estimatedCakeBountyReward: new BigNumber(
        estimatedCakeBountyReward as string
      ).toJSON(),
      totalPendingCakeHarvest: new BigNumber(
        totalPendingCakeHarvest as string
      ).toJSON(),
    };
  } catch (error) {
    return {
...
export const fetchVaultFees = async () => {
  try {
    const [performanceFee, callFee, withdrawalFee, withdrawalFeePeriod] =
      await makeBatchRequest([
