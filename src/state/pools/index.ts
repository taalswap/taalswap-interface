...
export const fetchPoolsPublicDataAsync =
  (currentBlock: number) => async (dispatch, getState) => {
    ...
    const liveData = poolsConfig.map((pool) => {
      ...
      const isPoolEndBlockExceeded =
        currentBlock > 0 && blockLimit
          ? currentBlock > Number(blockLimit.endBlock)
          : false;
...
export const fetchPoolsStakingLimitsAsync =
  () => async (dispatch, getState) => {
    const poolsWithStakingLimit = getState()
      .pools.data.filter(
        ({ stakingLimit }) =>
          stakingLimit !== null && stakingLimit !== undefined
      )
...
export const fetchCakeVaultPublicData = createAsyncThunk<CakeVault>(
...
export const fetchCakeVaultFees = createAsyncThunk<VaultFees>(
  'cakeVault/fetchFees',
  async () => {
...
export const fetchCakeVaultUserData = createAsyncThunk<
  VaultUser,
  { account: string }
>('cakeVault/fetchUser', async ({ account }) => {
...
export const PoolsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      ...
      (state, action: PayloadAction<CakeVault>) => {
        state.cakeVault = { ...state.cakeVault, ...action.payload };
      }
    );
    // Vault fees
    builder.addCase(
      fetchCakeVaultFees.fulfilled,
      (state, action: PayloadAction<VaultFees>) => {
    ...
    builder.addCase(
      ...
      (state, action: PayloadAction<VaultUser>) => {
        const userData = action.payload;
