...
import { Button, Modal } from 'taalswap-uikit'
import { getBalanceNumber, getFullDisplayBalance } from 'utils/formatBalance'
import TicketInput from 'views/Components/TicketInput'
import ModalActions from 'views/Components/ModalActions'
...
const BuyTicketModal: React.FC<BuyTicketModalProps> = ({ max, onDismiss }) => {
  ...
  return (
    <Modal title={t('Enter amount of tickets to buy')} onDismiss={onDismiss}>
      ...
      />
      <div>
        <Tips>{t('1 Ticket = %lotteryPrice% TAL', { lotteryPrice: LOTTERY_TICKET_PRICE })}</Tips>
      </div>
      <div>
        <Announce>{t('Ticket purchases are final. Your TAL cannot be returned to you after buying tickets.')}</Announce>
        <Final>{t('You will spend: %num% TAL', { num: cakeCosts(val) })}</Final>
      </div>
      <ModalActions>
