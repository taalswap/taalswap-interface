...
const EarningsCell: React.FC<EarningsCellProps> = ({
  pool,
  account,
  userDataLoaded,
}) => {
  ...
  const earnings = userData?.pendingReward
    ...
    : BIG_ZERO;
  // These will be reassigned later if its Auto TAL vault
  ...
  const {
    userData: { taalAtLastUserAction, userShares, lastUserActionTime },
    pricePerFullShare,
  } = useCakeVault();
