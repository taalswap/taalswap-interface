...
const VaultStakeModal: React.FC<VaultStakeModalProps> = ({
  ...
  const usdValueStaked =
    cakePriceBusd.gt(0) && stakeAmount
      ? formatNumber(new BigNumber(stakeAmount).times(cakePriceBusd).toNumber())
      : '';
  ...
  const handleWithdrawal = async (convertedStakeAmount: BigNumber) => {
    setPendingTx(true);
    const shareStakeToWithdraw = convertCakeToShares(
      convertedStakeAmount,
      pricePerFullShare
    );
    // trigger withdrawAll function if the withdrawal will leave 0.000001 TAL or less
    const triggerWithdrawAllThreshold = new BigNumber(1000000000000);
    const sharesRemaining = userShares.minus(
      shareStakeToWithdraw.sharesAsBigNumber
    );
    ...
    if (isWithdrawingAll) {
      cakeVaultContract.methods
        ...
        .on('error', (error) => {
          ...
          toastError(
            t('Error'),
            t('%error% - Please try again.', { error: error.message })
          );
          setPendingTx(false);
        });
    } else {
